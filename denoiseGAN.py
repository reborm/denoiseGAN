...
class denoiseGAN:
    model_name = 'denoiseGAN'
    def __init__(self, config, batch_size=1, input_height=512, input_width=512, input_channels=3, sess=None):
    ...
    def generator(self, input_x, reuse=False):
        with tf.variable_scope('generator'):
            if reuse:
                scope.reuse_variables()     #reuse_variable()来使变量重用，如果reuse开启，当检查到有相同的名字时，直接返回
                                            #那个有相同名字的变量而不是重新定义一个再复制值
            # down_sample here
            ...
            #mean：要生成的随机值的均值 stddev:要生成的随机值的标准偏差 seed:用于创建随机种子
            with slim.arg_scope([slim.conv2d_transpose],
                ...
                print(conv3)
                shuffle1 = tf.nn.relu(conv3) #64*2*2
                print(shuffle1)
                conv4 = slim.conv2d_transpose(shuffle1, 256, 3, 1, scope='g_conv4')
                shuffle2 = tf.nn.relu(shuffle1)
                print(shuffle2) 
                conv5 = slim.conv2d_transpose(shuffle2, 3, 3, 1, scope='g_conv5')
                print(conv5)

                #tf.add_to_collection(name, key)用来把一个value放入名称是name的集合中，组成一个列表
                #tf.get_collection(key, name=None)用来获取一个名称是'key'的集合中的所有列表
                #tf.GraphKeys.TRAINABLE_VARIABLES指神经网络中的参数
                #'generator'是scope
    ...
    def build_model(self):
        ...
        self.fake = self.generator(self.input_noise, reuse=False)
    ...
    def inference_loss(self, real, fake):
        ...
        content_loss = inference_mse_content_loss(real, fake)   #content_loss其实就是mse
    ...
    def train(self):
        ...
        # data/train/*.*
        data = glob(os.path.join(self.config.dataset_dir, 'train', self.config.train_set_clean, '*.*'))
        ...
        for epoch in range(self.config.epoches):
            ...
            for idx in range(batch_idxs):
                ...
                batch_files_noise = [single.replace('mean', 'real') for single in batch_files]
                batch_x= [get_images(batch_file, self.config.is_crop, self.config.fine_size, self.images_norm) for batch_file in batch_files]
                batch_x_noise = [get_images(batch_file, self.config.is_crop, self.config.fine_size, self.images_norm) for batch_file in batch_files_noise]
                batch_x = np.array(batch_x).astype(np.float32)
                batch_x_noise = np.array(batch_x_noise).astype(np.float32)

    
                if counter < 2e4:                      
                    _, content_loss, psnr = self.sess.run([self.srres_optim, self.content_loss, self.psnr], feed_dict={self.input_target:batch_x, self.input_noise:batch_x_noise})
                    end_time = time.time()
    ...
    def sample(self,epoch, idx):
        ...
        data = glob(os.path.join(self.config.dataset_dir, 'val', self.config.val_set_clean, '*.*'))
        data = data[:self.batch_size]
        data_noise = [single.replace('mean', 'real') for single in data]
        batch_x = [get_images(batch_file, self.config.is_crop, self.config.fine_size, self.images_norm) for batch_file in data]
        batch_x_noise = [get_images(batch_file, self.config.is_crop, self.config.fine_size, self.images_norm) for batch_file in data_noise]
        batch_x = np.array(batch_x).astype(np.float32)
        batch_x_noise = np.array(batch_x_noise).astype(np.float32)
        
        sample_images, psnr, input_noise = self.sess.run([self.fake, self.psnr, self.input_noise], feed_dict={self.input_target:batch_x, self.input_noise:batch_x_noise})


        save_images(sample_images, [4,4], './{}/{}_sample_{}_{}.png'.format(self.config.sample_dir, 'val',epoch, idx))
        save_images(input_noise, [4,4], './{}/{}_input_{}_{}.png'.format(self.config.sample_dir, 'val',epoch, idx))
        print('---------------------------------------')
    ...
    def test(self):
        ...
        test = glob(os.path.join(self.config.dataset_dir, 'test', self.config.test_dnd_noise, '*.*'))

        batch_idxs = int(len(test) / self.batch_size)

        for idx in range(batch_idxs):
            batch_files_noise = test[idx * self.batch_size:(idx + 1) * self.batch_size]

            batch_x = [get_images(batch_file, self.config.is_crop, self.config.fine_size, self.images_norm) for batch_file in batch_files_noise]
            batchs = np.array(batch_x).astype(np.float32)
        
            sample_images, input_noises = self.sess.run([self.fake, self.input_noise], feed_dict={self.input_noise:batchs})
            ...
            for i in range(len(batch_x)):
                ...
                input_noise = np.expand_dims(input_noises[i],0)
                #save_images(batch, [1,1], './{}/{}_gt_hr_{}.png'.format(self.config.test_dir, self.config.test_set,i))

                #save_images(sample_image, [1,1], './{}/{}_test_hr_{}.png'.format(self.config.test_dir, 'dnd_noise',i))
                #save_images(input_noise, [1,1], './{}/{}_gt_lr_{}.png'.format(self.config.test_dir, 'dnd_noise',i))

                #合并显示并且保存
                htitch = np.hstack((input_noise, sample_image))
                save_images(htitch, [1,1], './{}/{}/dnd_result_{}_{}.png'.format(self.config.test_dir, self.config.model_dir, idx, i))
...
if __name__=='__main__':
    denoise_gan = denoiseGAN()
    a = tf.random_normal([64,256,256,3])
    out = denoise_gan.generator(a)
